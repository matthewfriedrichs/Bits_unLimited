...
export default class BrushUI {
    ...
    renderModalContent() {
        ...
        const makeRange = (key, label, min, max, unit = '') => dom('div', {},
            dom('div', { class: "flex justify-between text-xs text-neutral-400 mb-1" }, dom('span', { class: "uppercase font-bold" }, label), dom('span', {}, `${b[key]}${unit}`)),
            dom('input', { type: 'range', min, max, step: 1, value: b[key], class: "w-full", onInput: (e) => { const val = parseInt(e.target.value); e.target.previousSibling.lastChild.innerText = `${val}${unit}`; this.updateProp(key, val); } })
        );
        ...
        const content = dom('div', { class: "bg-neutral-800 w-full max-w-xs rounded-xl shadow-2xl border border-neutral-700 overflow-hidden flex flex-col" },
            ...
            dom('div', { class: "p-5 flex flex-col gap-4" },
                ...
                makeRange('size', 'Size', 1, 32, 'px'),

                // NEW: Dynamics Sliders
                makeRange('angle', 'Angle', 0, 360, '°'),
                makeRange('angleJitter', 'Angle Jitter', 0, 180, '°'),
                makeRange('noise', 'Noise', 0, 100, '%'),
