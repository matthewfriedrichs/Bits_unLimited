...
export default class ToolbarUI {
    ...
    render() {
        ...
        if (!indicator) {
            indicator = dom('div', { id: 'active-tool-indicator', class: 'w-12 h-12 mb-4 relative shrink-0' });
    ...
    updatePenIcon() {
        const btn = document.getElementById('tool-btn-pen');
        if (!btn) return;
        const brush = this.store.get('activeBrush');
        const footprint = brush.footprint || [{ x: 0, y: 0 }];

        const canvas = dom('canvas', { width: 32, height: 32, class: 'w-8 h-8 image-pixelated' });
        const ctx = canvas.getContext('2d');
        const zoom = Math.min(4, 24 / Math.max(brush.size, 1));
        const cx = 16, cy = 16;

        ctx.fillStyle = '#ffffff';
        footprint.forEach(pt => {
            ctx.fillRect(
                Math.floor(cx + pt.x * zoom - zoom / 2),
                Math.floor(cy + pt.y * zoom - zoom / 2),
                Math.ceil(zoom), Math.ceil(zoom)
            );
        });

        const badge = dom('div', { class: 'absolute -top-1 -right-1 text-[10px] bg-neutral-900 rounded-full w-4 h-4 flex items-center justify-center border border-neutral-600 shadow-sm' }, DomBuilder.icon('pen', 'text-sky-400'));
        btn.innerHTML = ''; btn.appendChild(canvas); btn.appendChild(badge);
    }

    updateEraserIcon() {
        const btn = document.getElementById('tool-btn-eraser');
        if (!btn) return;
        const brush = this.store.get('eraserBrush');
        const footprint = brush.footprint || [{ x: 0, y: 0 }];
        ...
        const badge = dom('div', { class: 'absolute -top-1 -right-1 text-[10px] bg-neutral-900 rounded-full w-4 h-4 flex items-center justify-center border border-neutral-600 shadow-sm' }, DomBuilder.icon('eraser', 'text-rose-400'));
        btn.innerHTML = ''; btn.appendChild(canvas); btn.appendChild(badge);
    }

    updateBucketIcon(mode) {
        const btn = document.getElementById('tool-btn-bucket');
        if (!btn) return;
        btn.innerHTML = '';
        btn.appendChild(DomBuilder.icon('fill-drip', 'text-amber-400'));
        if (mode === 'smart') {
            const badge = dom('div', { class: 'absolute -top-1 -right-1 text-[10px] bg-neutral-900 rounded-full w-4 h-4 flex items-center justify-center border border-neutral-600 shadow-sm' }, DomBuilder.icon('wand-magic-sparkles', 'text-fuchsia-400'));
            btn.appendChild(badge);
        }
    }

    updateSelectIcon(mode) {
        const btn = document.getElementById('tool-btn-select');
        if (!btn) return;
        btn.innerHTML = '';
        const icon = mode === 'color' ? 'magic' : 'shapes';
        const color = mode === 'color' ? 'text-fuchsia-400' : 'text-sky-400';
        const badge = dom('div', { class: 'absolute -top-1 -right-1 text-[10px] bg-neutral-900 rounded-full w-4 h-4 flex items-center justify-center border border-neutral-600 shadow-sm' }, DomBuilder.icon(icon, color));
        btn.appendChild(badge);
        btn.appendChild(DomBuilder.icon('vector-square', 'text-green-400'));
    }

    renderSplitIcon(pId, sId) {
        this.indicatorEl.innerHTML = '';
        const getToolDef = (id) => this.toolsDef.find(t => t.id === id) || this.toolsDef[0];
        const pTool = getToolDef(pId);
        const sTool = getToolDef(sId);
        ...
        if (pId === sId) {
            const el = dom('div', { class: commonClass }, DomBuilder.icon(pTool.icon, pTool.color));
            this.indicatorEl.appendChild(el);
        } else {
            const pEl = dom('div', {
                class: commonClass + " z-10 bg-neutral-800",
                style: { clipPath: 'polygon(0 0, 100% 0, 0 100%)' }
            }, dom('div', { class: '-translate-x-1 -translate-y-1' }, DomBuilder.icon(pTool.icon, pTool.color)));

            const sEl = dom('div', {
                class: commonClass + " z-0 bg-neutral-700",
                style: { clipPath: 'polygon(100% 0, 100% 100%, 0 100%)' }
            }, dom('div', { class: 'translate-x-1 translate-y-1' }, DomBuilder.icon(sTool.icon, sTool.color)));

            const border = dom('div', {
                class: "absolute inset-0 pointer-events-none z-20 border-white/10",
                style: { background: 'linear-gradient(135deg, transparent 49.5%, #404040 49.5%, #404040 50.5%, transparent 50.5%)' }
            });

            this.indicatorEl.appendChild(sEl);
            this.indicatorEl.appendChild(pEl);
            this.indicatorEl.appendChild(border);
        }

        const labels = dom('div', { class: "absolute -bottom-2 w-full flex justify-between text-[8px] font-bold px-1 text-neutral-500 uppercase pointer-events-none z-30" },
            dom('span', {}, 'L'),
            dom('span', {}, 'R')
        );
    ...
    updateListHighlights(pId, sId) {
        document.querySelectorAll('.tool-btn').forEach(btn => {
            ...
            const isP = btn.id === `tool-btn-${pId}`;
            const isS = btn.id === `tool-btn-${sId}`;
    ...
    startHoverTimer(toolId, btn) {
        ...
        // FIX: Reduced delay to 100ms for snappier response
            ...
            tool.availableModes.forEach(mode => {
                ...
                const modeBtn = dom('button', {
                    ...
                    onClick: () => {
                        ...
                        // FIX: Re-render to show update immediately
        ...
        if (hasSettings) {
            tool.settings.forEach(setting => {
                ...
                if (setting.type === 'brush-picker') {
                    ...
                    input = dom('div', {
                        class: "flex items-center justify-between bg-neutral-700 p-1 rounded cursor-pointer",
                        onClick: () => {
                            tool.setSetting(setting.id, !setting.value);
                            // FIX: Re-render to update toggle state visually
                            this.showOptionsPanel(toolId, btnElement);
                        }
                    },
